...
def main() -> int:
    ...
    fewer_files = (rr800 and prh100 and rr800["fileCountMedian"] and
                   prh100["fileCountMedian"] and
                   rr800["fileCountMedian"] * 100 < prh100["fileCountMedian"])
    crit("materially fewer filesystem objects than PeakRDL-html", fewer_files,
         f"regreview 800k: {int(rr800['fileCountMedian'])} file(s); "
         f"peakrdl-html 100k: {int(prh100['fileCountMedian'])} files"
         if rr800 and prh100 else "no data")
    ...
    # --- hardware/env ---
    hw = {}
    for r in RAW.glob("*regreview-800k*.json"):
        hw = load(r).get("hardware", {})
        versions = load(r).get("runtimeVersions", {})
        break
