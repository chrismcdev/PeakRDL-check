...
def main() -> int:
    ...
    doc = f"""# PROOF — the evidence behind PeakRDL-check's claims
...
**What this document is.** The two claims below were set up as falsifiable
hypotheses during development and every criterion was measured rather than
asserted. They are not requirements you need to care about to use the tool —
they are the receipts for what the README says the tool does well. Keep this
dossier around so "faster", "scales" and "detects breaking changes" stay
verifiable statements instead of marketing.
