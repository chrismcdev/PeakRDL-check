"""peakrdl-check command-line interface."""
...
def cmd_benchmark(args) -> int:
    ...
    cmd = [sys.executable, str(script), "--fixture", args.fixture,
           "--runs", str(args.runs)]
    if args.compare:
        cmd += ["--tools", f"peakrdl-check,{args.compare}"]
    else:
        cmd += ["--tools", "peakrdl-check"]
    return subprocess.call(cmd)


def main(argv=None) -> int:
    ap = argparse.ArgumentParser(prog="peakrdl-check")
    ...
    s.add_argument("--changes", default=None,
                   help="changes.json from `peakrdl-check diff` to display")
